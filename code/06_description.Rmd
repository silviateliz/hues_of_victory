...
- The SaberVotar data has a lower proportion of missing data. Therefore, a possible solution is to impute skin color data from the SaberVotar profile to the INE profile. The final skin color measure could be the average of both measures.
...
```{r av_skin}
# Obtain average skin tone from both skin tone measures. If one measure is absent, use only existing measure. If both measures are absent, use NA.
data %<>%
  mutate(av_skintone = ifelse(is.na(skin_ine) & is.na(skin_sv), NA,
...
```{r missing_skin}
missing_skin <- data %>%
  filter(sv_d == 1 & ine_d == 1) %>%
  summarize(n = n(),
            n_na = sum(is.na(av_skintone)),
            prop_existing = round(n_na / n(), 5),
            prop_total = round(n_na / nrow(data), 5))
...
- The skin tone classification algorithm was not able to measure the skin tone of `r missing_skin$n_na` candidates. This number represents `r round(missing_skin$prop_existing*100, 2)`% of the candidates with image data and `r round(missing_skin$prop_total*100, 2)`% of the total candidates in the dataset. This issue may be resolved by adjusting the algorithm's facial detection parameters.
- The overall skin tone distribution is slightly left-skewed, indicating that most candidates have a medium-dark skin tone. The mean skin tone is `r round(mean(data$av_skintone, na.rm = TRUE), 2)` and the standard deviation is `r round(sd(data$av_skintone, na.rm = TRUE), 2)`. The skin tone distribution is shown in the following figure.

```{r skin_describe}
# Plot skin tone distribution
data %>%
  ggplot(aes(x = av_skintone)) +
  geom_histogram(bins = 10, fill = "gray50", color = "white") +
  geom_vline(xintercept = 7.1, linetype = "solid", color = "black", size = 0.8) +
  annotate("text", x = 7.1, 
           y = 200, label = "Mean = 7.1", 
           vjust = -2, hjust = -0.3, size = 3) +
  annotate("text", x = 7.1, y = 200, 
           label = "SD = 1.72", 
           vjust = -.5, hjust = -0.3, size = 3) +
  labs(title = "Skin tone distribution",
       x = "Skin tone",
       y = "Count") +
...
skin_state <- data %>%
  ...
  summarize(
    n = n(),
    mean = round(mean(av_skintone, na.rm = TRUE), 2),
    sd = round(sd(av_skintone, na.rm = TRUE), 2),
    skintone_dist = list(av_skintone)) 
