...
all_candidates %<>% select(-c("CIRCUNSCRIPCION", "MUNICIPIO", "NUM_LISTA_O_FORMULA", "ESTATUS", "DIRECCION_CASA_CAMPAÑA", "TELEFONO", "CORREO_ELECTRONICO", "PAGINA_WEB", "REDES", "ESCOLARIDAD", "ESTATUS_ESCOLARIDAD", "CURSOS", "HISTORIA_PROFESIONAL", "TRAYECTORIA_POLITICA", "MOTIVO_CARGO_PUBLICO", "PROPUESTA_1", "PROPUESTA_2", "PROPUESTA_GENERO"))

names(all_candidates) <- c("party", "position", "state", "district", "name", "type", "age", "sex")
...
# Recode sex
all_candidates$sex %<>% fct_recode("f" = "mujer", "m" = "hombre")
...
```{r unique_id}
# Create a unique_id column that concatenates the state_id, district_id, and the first five letters of the name column.
all_candidates <- all_candidates %>%
  mutate(unique_id = paste0(state_id, district_id, substr(name, 1, 4), substr(party, 1, 3)))
```
...
dep_candidates %>% count(unique_id) %>% filter(n > 1)
```
...
sen_candidates %>% count(unique_id) %>% filter(n > 1)
```
...
pres_candidates %>% count(unique_id) %>% filter(n > 1)
```
