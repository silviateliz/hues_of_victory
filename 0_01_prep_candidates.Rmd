...
all_candidates <- read_excel("data/raw_files/candidate_list/baseDatosCandidatos.xls")
...
names(all_candidates) <- c("party", "position", "state", "district", "name", "profile_url", "type", "estatus", "age", "sex", "webpage", "social_media", "prop_1", "prop_2", "prop_gender")

# Turn all content to lowercase
all_candidates %<>% mutate_all(tolower)
...
```{r portrait_url}
# Extract the number in "profile_url" that lies in between "detallecandidato/" and "/"
all_candidates$candidate_num <- as.numeric(str_extract(all_candidates$profile_url, "(?<=detallecandidato\\/)\\d*(?=\\/\\d)"))

# Create a new column "portrait_url" 
all_candidates$portrait_url <- paste0("https://candidaturas.ine.mx/cycc/img/fotocandidato/", all_candidates$candidate_num, ".jpg")
...
all_candidates$portrait_d <- ifelse(all_candidates$estatus=="sin captura", 0, 1)
...
# Separate social_media column into different columns based on string separator ","
#all_candidates <- all_candidates %>%
 # separate(social_media, into = c("sm_1", "sm_2", "sm_3", "sm_4", "sm_5"), sep = ",", remove = F)

# Replace "no proporcionó" with NA
all_candidates$social_media <- ifelse(all_candidates$social_media == "no proporcionó", NA, all_candidates$social_media)
#all_candidates$sm_1 <- ifelse(all_candidates$sm_1 == "no proporcionó", NA, all_candidates$sm_1)
```
